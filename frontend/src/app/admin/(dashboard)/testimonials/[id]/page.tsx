...
interface Translation { locale: string; name: string; role: string; content: string; avatar: string }
...
export default function TestimonialEditPage() {
  ...
  const [translations, setTranslations] = useState<Record<string, Translation>>({
    mn: { locale: 'mn', name: '', role: '', content: '', avatar: '' },
    en: { locale: 'en', name: '', role: '', content: '', avatar: '' },
  });
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (isNew) return;
    fetch(`/api/admin/testimonials/${id}`).then((r) => r.json()).then((data) => {
      ...
      for (const tr of t.translations || []) trans[tr.locale] = tr;
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input value={translations[locale]?.role || ''} onChange={(e) => updateTrans('role', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Content ({locale.toUpperCase()})</label>
              <textarea value={translations[locale]?.content || ''} onChange={(e) => updateTrans('content', e.target.value)} rows={4} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Avatar ({locale.toUpperCase()})</label>
              <input value={translations[locale]?.avatar || ''} onChange={(e) => updateTrans('avatar', e.target.value)} placeholder="Initials or URL" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => router.push('/admin/testimonials')} className="px-6 py-2 border border-gray-300 rounded-lg font-medium hover:bg-gray-50">Cancel</button>
        </div>
      </form>
    </div>
  );
